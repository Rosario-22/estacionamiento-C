...
/**
 * Representa un auto dentro del estacionamiento.
 */
typedef struct {
    int horaIngreso;
    char patente[N];
 ...
 * Contiene un auto y un puntero al siguiente nodo.
 */
typedef struct Nodo {
    Auto data;
    struct Nodo* sgte;
...
/**
 * Representa el estacionamiento.
 * Mantiene información de capacidad y punteros a los extremos de la lista.
 */
typedef struct {
    int longitud;
    int capacidad;
...
void mostrarAutos(tEstacionamiento* E) {
    ...
    while (aux != NULL) {
        printf("Patente: %s | Hora: %d | Ubicacion: %d\n",
               aux->data.patente,
